...
test(int fftLength, int profilingIterations, int numOfIterationsToSkip)
{
  ...
  double* dInOut = (double*)pffftd_aligned_malloc(fftLength * sizeof(double));
  ...
  float* fInOut = (float*)pffft_aligned_malloc(fftLength * sizeof(float));

  for (int i = 0; i < fftLength; ++i) {
    oInOut[i] = dInput[i] = dInOut[i] =
      sin(2.0 * M_PI * 0.45 * (double)i * (1.0 - ((double)i) / (2.0 * (double)fftLength)));
    fInOut[i] = fInput[i] = (float)dInput[i];
  }
  ...
  for (int i = 0; i < profilingIterations; ++i) {
    ...
    fftDouble.Forward(dInOut, dInOut);
    fftDouble.Inverse(dInOut, dInOut);
  ...
  for (int i = 0; i < profilingIterations; ++i) {
    ...
    fftFloat.Forward(fInOut, fInOut);
    fftFloat.Inverse(fInOut, fInOut);
  ...
  for (int i = 0; i < fftLength; ++i) {
    ...
    double dDiff = dInput[i] - dInOut[i];
    dNoisePower += dDiff * dDiff;

    double fDiff = fInput[i] - fInOut[i];
  ...
  pffftd_aligned_free(dInOut);
  pffft_aligned_free(fInput);
  pffft_aligned_free(fInOut);
  pffft_aligned_free(oInOut);
}
